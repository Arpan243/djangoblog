...
import os
from acqux.settings import MEDIA_URL
...
def new_post(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            return redirect(f"http://127.0.0.1:8000/{form.cleaned_data['series'].slug}")
...
def newsletter(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail = EmailMessage(subject, email_message, f"acqux <{request.user.email}>", bcc=receivers)
